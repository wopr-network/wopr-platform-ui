...
const MOCK_ORG = {
	...
	members: [
		{ userId: "e2e-user-id", role: "admin", email: "e2e@wopr.test" },
	],
	invites: [],
};
...
async function mockPaymentAPI(
	...
	await page.route(
		...
		async (route) => {
			...
			const results = procs.map((proc) => {
					...
					try {
						...
						if (id) {
							state.billingInfo.paymentMethods =
								state.billingInfo.paymentMethods.filter(
									(pm) => pm.id !== id,
								);
						}
					} catch {
				...
				if (
					proc === "billing.setDefaultPaymentMethod" &&
					method === "POST"
				) {
					try {
						const body = route.request().postDataJSON();
						...
						if (id) {
							state.billingInfo.paymentMethods =
								state.billingInfo.paymentMethods.map((pm) => ({
									...pm,
									isDefault: pm.id === id,
								}));
						}
	...
	// REST: setup-intent
	await page.route(
		`${PLATFORM_BASE_URL}/api/billing/setup-intent`,
		async (route) => {
			await route.fulfill({
				status: 200,
				contentType: "application/json",
	...
	// REST: dividend stats
	await page.route(
		`${PLATFORM_BASE_URL}/api/billing/dividend/stats`,
		async (route) => {
			await route.fulfill({
				status: 200,
				contentType: "application/json",
...
test.describe("Payment Methods Page", () => {
	test("payment page loads and shows existing payment methods", async ({
		authedPage: page,
	}) => {
	...
	test("set a non-default card as default", async ({
		authedPage: page,
	}) => {
		const state = createMockState();
		await mockPaymentAPI(page, state);
		...
		const mc5555 = page.getByText("**** **** **** 5555");
	...
	test("add payment method dialog opens and shows Stripe Elements", async ({
		...
		await expect(
			stripeFrame
				.first()
				.locator('[name="number"], [name="cardNumber"], input')
				.first(),
	...
	test("empty state shows no-payment-methods message", async ({
		authedPage: page,
	}) => {
		const state = createMockState();
		state.billingInfo.paymentMethods = [];
		...
		await expect(
			page.getByText("No payment methods on file."),
		).toBeVisible();
	});
	...
	test("error state shows retry button", async ({
		...
		await expect(
			page.getByText("Failed to load billing information."),
		).toBeVisible({ timeout: 10000 });
